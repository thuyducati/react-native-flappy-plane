...
export const addPipesAtLocation = (x, world, entities) => {
    ...
    let pipeTopWidth = Constants.PIPE_WIDTH + 10
    ...
    Object.keys(entities).forEach(key => {
        if (key.indexOf("pipe") === 0 && entities.hasOwnProperty(key)) {
            Matter.Body.translate(
                entities[key].body,
                {
                    x: -10, // velocity of the pipes // edit to -10 before release
                    y: 0 // require 0
                }
            )
            ...
            if (key.indexOf("Top") !== -1 && parseInt(key.replace("pipe", "")) % 2 === 0) {
                ...
                // rendering the pipes from the third one
            ...
            if (entities[key].body.position.x <= -1 * (Constants.MAX_WIDTH / 2)) {
                ...
                Matter.Body.translate(
                    entities[key].body,
                    {
                        x: -10, // velocity of the floor
                        y: 0 // keep the floor at the bottom and require 0
                    }
                )
            }
        }
